...
export async function POST(request: NextRequest) {
  ...
  try {
    const { mailId, accountId } = await request.json();
    ...
    if (isExpired && account.encrypted_refresh_token) {
      try {
        ...
        console.error(`Failed to refresh token for trashing:`, refreshErr);
      }
    }
    ...
    if (account.provider === "google") {
      ...
      const res = await fetch(
        `https://gmail.googleapis.com/gmail/v1/users/me/messages/${googleMsgId}/trash`,
        {
          method: "POST",
          headers: {
      ...
      if (!res.ok) {
        ...
        console.error("Gmail trash error response:", errText);
        throw new Error(`Gmail trash API failed with status ${res.status}`);
      }
    } else if (account.provider === "microsoft") {
      const microsoftMsgId = mailId.replace("microsoft_", "");
      const res = await fetch(
        `https://graph.microsoft.com/v1.0/me/messages/${microsoftMsgId}/move`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      if (!res.ok) {
        const errText = await res.text();
        console.error("Outlook trash error response:", errText);
        throw new Error(`Microsoft trash API failed with status ${res.status}`);
      }
    }

    return jsonOk({ message: "E-mail succesvol verwijderd." });
  } catch (error) {
    console.error("Error deleting mail route:", error);
    return jsonError("Fout bij het verwijderen van e-mail.", 500);
  }
}
